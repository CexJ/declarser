...
public class InjectionTypeException extends FunctionTypeException {
    ...
    public <T> InjectionTypeException(Collection<T> first, Collection<T> second, Collection<T> delta) {
    ...
    public static <T> InjectionTypeException of(Collection<T> first, Collection<T> second) {
        final var delta = first.stream()
                .filter(f -> ! second.contains(f))
                .collect(Collectors.toSet());
        return new InjectionTypeException(first, second, delta);
    }
