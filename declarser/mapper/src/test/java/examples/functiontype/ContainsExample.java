...
import static kernel.enums.FunctionType.INJECTIVE;
...
public class ContainsExample {
    ...
    public void mapping_in_bijection_return_success(){
        ...
        final var built = MapperDeclarserBuilder.from(From2.class).to(To2.class).withToFields(CONTAINS).build();
    ...
    public void mapping_contains_class_return_success(){
        ...
        final var built = MapperDeclarserBuilder.from(From1.class).to(To2.class).withToFields(CONTAINS).build();
        assertTrue(built.isSuccess());
        final var declarser = built.getValue();
        final var result = declarser.apply(from);
        assertTrue(result.isSuccess());
    ...
    public void mapping_contained_class_return_success(){
        ...
        final var value2 = new FieldType2();
        final var from = new From2(value1,value2);
        final var built = MapperDeclarserBuilder.from(From2.class).to(To1.class).withToFields(CONTAINS).build();
        assertTrue(built.isSuccess());
        final var declarser = built.getValue();
        final var result = declarser.apply(from);
    ...
    public void mapping_with_outsiders_return_success(){
        ...
        final var built = MapperDeclarserBuilder.from(FromWithOutsider.class).to(ToWithOutsider.class).withToFields(CONTAINS).build();
        assertTrue(built.isSuccess());
        final var declarser = built.getValue();
        final var result = declarser.apply(from);
        assertTrue(result.isFailure());
