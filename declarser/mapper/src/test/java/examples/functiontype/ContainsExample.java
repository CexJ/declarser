...
public class ContainsExample {
    ...
    public void mapping_in_bijection_return_success(){
        ...
        final var built = MapperDeclarserBuilder.from(From2.class).to(To2.class).withTargetFields(CONTAINS).build();
    ...
    public void mapping_contains_class_return_success(){
        ...
        final var built = MapperDeclarserBuilder.from(From1.class).to(To2.class).withTargetFields(CONTAINS).build();
        assertTrue(built.isSuccess());
        final var declarser = built.getValue();
        final var result = declarser.apply(from);
        assertTrue(result.isSuccess());
    ...
    public void mapping_contained_class_return_failure(){
        final var built = MapperDeclarserBuilder.from(From2.class).to(To1.class).withTargetFields(CONTAINS).build();
        assertTrue(built.isFailure());
    }

    @Test
    public void mapping_with_outsiders_return_failure(){
        final var built = MapperDeclarserBuilder.from(FromWithOutsider.class).to(ToWithOutsider.class).withTargetFields(CONTAINS).build();
        assertTrue(built.isFailure());
    }
}
